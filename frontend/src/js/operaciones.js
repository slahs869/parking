...
function tarifaCarroFunction() {
  ...
  // console.log(carroPrecio,mensajeCarro)
}
...
async function mostrarTexto() {
  ...
  //console.log(placa,hora,id,precio,checkout,message)
  return [value, placa, hora, id, precio, checkout, message, position];
...
async function mostrarFactura() {
  ...
  if (checkout == "INGRESADO") {
    console.log("hola")
  }



  if (value == 1 && checkout == "INGRESADO") {
    botonImp.innerHTML = ''
    //CONVERSIONES
  ...
  if (mensajeM == "Moto  ") {
    if (motoPrecio != 0) {
      if (textosalida == 'RETIRADO') {
        console.log(entradaa, Date(), precioM, mensajeM)
        await agregar({
          placa: `${entradaa}`,
          fecha: `${Date()}`,
          estado: "INGRESADO",
